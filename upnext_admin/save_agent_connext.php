...
$check = mysqli_query($conn,"SELECT firstname,lastname from agent_list_tb where firstname ='$fname' and lastname = '$lname' ");
$checkdata = mysqli_num_rows($check);

if ($checkdata>0) {
	 echo '<script>alert("Data Already Exist !");
	 window.location.href = "add_agent.php";</script>';
}else{
	$sql = "INSERT INTO `agent_list_tb`(`lastname`, `firstname`, `middlename_suffix`, `house_unit_no`, `street_no`, `floor_bldg_subdiv_no`, `barangay`, `city_town`, `province`, `zipcode`, `tel_no`, `mob_no`, `email`, `month`, `day`, `year`, `age`, `complete_name`, `cr_tel_no`, `cr_mob_no`, `question`, `aub_atm_no`, `aub_act_no`, `program`,`program_tnvs`, `branch_assign`, `source_code`, `endorse_by`, `remarks`, `name_of_approval`, `date_approve`, `username`, `password`, `status`,`acct_approval`,`account_activation`,`agent_type`)VALUES('$lname','$fname','$mname','$h_unit','$streetname','$floor','$brgy','$city','$province','$zip','$tel1','$mob1','$email','$dob','$day','$yr','$age','$comname','$tel2','$mob2','$client','$aub','$aub_acct','$prog','$prog_tnvs','$branch','$source','$endorse','$remarks','$approver','$date_app','$_SESSION[new_name]','$password','Inactive','Pending','Not yet Activated','connext')";
